...
3. **Write it down.** From whatever they give, set the real values in `overrides/brand.css` (uncomment + fill `--brand-primary`, `--brand-secondary`/`--brand-accent` where known, `--brand-font`, `--signature`), confirm in one line, then move to GATE 1. The whole system re-derives from these — get this right and every component, template and card shows their brand instead of the default LinkedIn blue.
