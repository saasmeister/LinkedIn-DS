...
2. **Know the brand.** Read `overrides/brand.css`. Configured → proceed. Unconfigured (all-commented) → ask the brand essentials in one short turn (primary colour, optional secondary/accent, font) or offer to proceed with sensible neutral defaults; if the user pastes brand CSS, treat it as their brand layer. No setup wizard exists in this variant.
...
## ⚙️ Brand setup (conversational — no wizard in this variant)

Brand setup is the brand layer (`overrides/brand.css`): primary / secondary / accent / tint / font / signature. In this headless variant there is no `START HERE.html` screen, so:

- **Read `overrides/brand.css` first.** Real values → brand is configured, go to the brief. All-commented → it's unconfigured.
- **If unconfigured, don't derail into a long setup interview.** Ask the essentials in one line (primary colour + font; offer to derive a secondary from one colour), or offer sensible neutral defaults so you can proceed. The content layer — `overrides/voice.md`, `icp.md`, `offer.md` — shapes example copy; read or fill it so copy lands in the client's voice, not generic SaaS filler.
- **If the user pastes brand CSS,** treat it as the brand layer for this session (and write it to `overrides/brand.css` if the host allows file writes).
